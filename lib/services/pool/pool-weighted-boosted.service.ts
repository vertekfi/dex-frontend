...
export class PoolWeightedBoostedService implements PoolService {
  ...
  public async joinGetContractCallData(data: PoolJoinData): Promise<PoolJoinContractCallData> {
    ...
    const calls: string[] = [];
    const { swaps, assets, deltas, tokensIn, tokensOut } = await this.getJoinSwaps(
      data.tokenAmountsIn,
    );
  ...
  public async joinGetProportionalSuggestionForFixedAmount(
    ...
    if (tokenAmountsMappedToPoolTokens.length !== 1) {
      throw new Error(
        'PoolWeightedBoostedService: Incorrect token amounts returned by getJoinSwaps',
      );
    }

    //these suggestions are now in the pool tokens (base + phantom bpts)
    const proportionalSuggestions =
      await this.weightedPoolService.joinGetProportionalSuggestionForFixedAmount(
    ...
    const exitAmounts = proportionalSuggestions.map((suggestion) => {
      ...
      const tokenOption = option.tokenOptions.find((tokenOption) =>
        tokensIn.includes(tokenOption.address),
      )!;
  ...
  public async joinGetBptOutAndPriceImpactForTokensIn(
    ...
    const bptAmount = this.weightedPoolService.exactTokensInForBPTOut(
      tokenAmountsMappedToPoolTokens,
    );
    ...
    const bptZeroPriceImpact = this.weightedPoolService.bptForTokensZeroPriceImpact(
      tokenAmountsMappedToPoolTokens,
    );
  ...
  public async exitGetContractCallData(data: PoolExitData): Promise<PoolExitContractCallData> {
    ...
    const batchSwapEntries = phantomBptTokensWithExits.map((poolToken) => {
      const exitAmountOut = exitAmountsOut.find(
        (amountOut) => amountOut.address === poolToken.address,
      )!;
      const option = this.pool.withdrawConfig.options.find(
        (option) => option.poolTokenIndex === poolToken.index,
      )!;
      const tokenOut = option.tokenOptions.find((tokenOption) =>
        tokensOut.includes(tokenOption.address),
      )!;
  ...
  public async exitGetBptInForSingleAssetWithdraw(
    ...
    return this.weightedPoolService.exitGetBptInForSingleAssetWithdraw(
      tokenAmountsMappedToPoolTokens[0],
    );
  }
  ...
  public async exitGetSingleAssetWithdrawForBptIn(
    ...
    const { tokenOutAmounts } = await this.getExitSwaps([
      {
        address: poolToken.address,
        amount: weightedExitData.tokenAmount,
  ...
  public async exitGetProportionalWithdrawEstimate(
    ...
    const exitAmounts = withdrawAmounts
      ...
      .map(({ amount, address }) => {
        const option = this.pool.withdrawConfig.options.find(
          (option) => option.poolTokenAddress === address,
        )!;
        const tokenOption = option.tokenOptions.find((tokenOption) =>
          tokensOut.includes(tokenOption.address),
        )!;
  ...
  private async getJoinSwaps(tokenAmountsIn: TokenAmountHumanReadable[]): Promise<{
    ...
    for (const tokenAmountIn of tokenAmountsIn) {
      ...
      if (
        ...
        joinSwaps.push({
          swaps: swaps.slice(0, swaps.length - 1),
  ...
  private encodeJoinPool({
    ...
    const amountsIn = this.pool.tokens.map((token) => {
      const tokenAmountIn = data.tokenAmountsIn.find(
        (tokenAmountIn) => tokenAmountIn.address === token.address,
      );
      ...
      const index =
        batchSwapAssets.findIndex((asset) => asset.toLowerCase() === token.address) || -1;
    ...
    return this.batchRelayerService.vaultEncodeJoinPool({
      ...
      poolKind: 0,
      sender: data.userAddress,
      recipient: data.zapIntoMasterchefFarm
      ...
      joinPoolRequest: {
        ...
        userData: WeightedPoolEncoder.joinExactTokensInForBPTOut(
          amountsIn,
          parseUnits(data.minimumBpt, 18),
        ),
        fromInternalBalance,
      },
      value: joinHasNativeAsset ? ethAmountScaled : Zero,
      outputReference: data.zapIntoMasterchefFarm
  ...
  private async getExitSwaps(
    exitAmounts: (TokenAmountHumanReadable & { tokenOut: string })[],
  ): Promise<{
    swaps: SwapV2[];
    assets: string[];
    deltas: AmountScaledString[];
    ...
    for (const exitAmount of exitAmounts) {
      const poolToken = this.pool.tokens.find(
        (poolToken) => poolToken.address === exitAmount.address,
      );
  ...
  private phantomBptPoolTokenGetExitSwaps({
    ...
    throw new Error(
      `No available join swap path for poolId: ${poolToken.address} and token: ${tokenOut}`,
    );
  }
