...
export class PoolComposableJoinService {
  ...
  public async processJoinSteps({
    ...
    for (const step of this.joinSteps) {
      const joinAmountsIn = currentTokenAmountsIn.filter((amountIn) =>
        step.tokensIn.includes(amountIn.address),
      );
    ...
    return {
      ...
      nestedPriceImpacts: processedJoinSteps
        .slice(0, processedJoinSteps.length - 1)
        .map((step) => ({
          ...step,
          poolId: step.pool.id,
        })),
  ...
  private encodeJoinPool({
    ...
    const joinHasNativeAsset =
      wethIsEth && pool.tokens.find((token) => token.address === this.wethAddress);
    ...
    const amountsIn = tokensWithPhantomBpt.map((token) => {
      ...
      const tokenAmountIn = step.tokenAmountsIn.find(
