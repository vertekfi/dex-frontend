...
export class PoolComposableExitService {
    ...
    if (singleAssetExit.exitSwaps) {
      const requiresUnwrap =
        ...
        parseFloat(tokenAmount.amount) >
          parseFloat(singleAssetExit.linearPool.mainToken.totalBalance);
      ...
      if (requiresUnwrap && singleAssetExit.linearPool) {
        ...
        const wrappedTokenAmount: AmountHumanReadable = formatFixed(
          oldBnumScaleAmount(tokenAmount.amount, tokenOut.decimals)
            .div(wrappedToken.priceRate)
            .toFixed(0),
          tokenOut.decimals,
        );
  ...
  public async exitExactBPTInForOneTokenOutGetContractCallData(
    ...
    if (singleAssetExit.exitSwaps) {
      ...
      const { deltas, requiresUnwrap, swaps, assets, tokenAmountOut } =
        await this.getSingleAssetExitSwaps({
          exit: singleAssetExit,
          tokenOutAddress,
          amountIn,
        });
      ...
      calls.push(
        this.batchRelayerService.vaultEncodeExitPool({
          ...
          exitPoolRequest: this.getExitPoolRequest({
            ...
            inputReference:
              references.find((ref) => ref.address === nestedStablePoolToken.address)?.index ||
              null,
          }),
          outputReferences,
        }),
  ...
  private async getExitSwapsForNestedLinearPools(
    ...
    const exitSwaps: {
      swap: SwapV2;
      assets: string[];
      nestedLinearPool: ComposablePoolExitNestedLinearPool;
    }[] = [];
    const requiresUnwrap = !!nestedLinearPools.find(({ linearPoolToken, mainToken }) => {
      ...
      return (
        ...
        parseFloat(bptAmount?.amount || '0') * parseFloat(linearPoolToken.pool.bptPriceRate || '0')
      );
    });
    ...
    for (const nestedLinearPool of nestedLinearPools) {
      ...
      if (bptAmount) {
        exitSwaps.push({
          ...
          assets: [
            linearPoolToken.address,
            requiresUnwrap ? wrappedToken.address : mainToken.address,
          ],
          nestedLinearPool,
        });
      }
  ...
  private buildSingleAssetExit({
    ...
    const poolToken = this.pool.tokens.find(
      (poolToken) => poolToken.address === option.poolTokenAddress,
    )!;
    ...
    const exitSwapPool = this.isStablePool
      ? this.pool
      : 'pool' in poolToken
      ? poolToken.pool
      : null;
  ...
  private getSingleAssetExitForTokenOut(tokenOutAddress: string): ComposablePoolSingleAssetExit {
    const singleAssetExit = this.singleAssetExits.find(
      (exit) => exit.tokenOut.address === tokenOutAddress,
    );
  ...
  private async getSingleAssetExitSwaps({
    ...
    const swapTokenOut =
      requiresUnwrap && exit.linearPool ? exit.linearPool.wrappedToken : exit.tokenOut;
    const wrappedTokenPriceRate = exit.linearPool?.wrappedToken.priceRate || '1.0';

    const tokenAmountOut = formatFixed(
      BigNumber.from(deltas[assetOutIdx] || '0').abs(),
