...
export class AllowancesConcern {
  constructor(private readonly jsonProvider: BaseProvider) {}
    ...
    try {
      const allContractAllowances = await Promise.all(
        contractAddresses.map((contractAddress) =>
          this.getForContract(account, contractAddress, tokens),
        ),
      );
  ...
  public async getForContract(
    ...
    return Object.fromEntries(
      tokens.map((token, i) => [
        ...
        formatUnits(allowances[i], token.decimals),
      ]),
    );
  }
}
