...
export function useBatchSwap() {
  ...
  function batchSwap({
    ...
    const limits = tokenAddresses.map((tokenAddress) => {
      if (swapType === 'EXACT_IN') {
        if (
          ...
          (isEth(tokenIn) && tokenAddress === AddressZero)
        ) {
          return parseUnits(tokenInAmount, tokenInDefinition.decimals).toString();
        } else if (
          isSameAddress(tokenAddress, tokenOut) ||
          (isEth(tokenOut) && tokenAddress === AddressZero)
        ) {
    ...
    submit({
      ...
      toastText: `${tokenFormatAmount(tokenInAmount)} ${
        tokenInDefinition.symbol
      } -> ${tokenFormatAmount(tokenOutAmount)} ${tokenOutDefinition.symbol}`,
      ...(isEth(tokenIn)
        ? {
            overrides: {
              value:
