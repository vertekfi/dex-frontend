...
export function usePoolGetMaxProportionalInvestmentAmount() {
  ...
  const tokenOptionsWithHighestValue: TokenAmountHumanReadable[] = pool.investConfig.options.map(
    (option) => {
      const tokenWithHighestValue = orderBy(
        option.tokenOptions,
        (tokenOption) =>
          priceForAmount({ ...tokenOption, amount: getUserBalanceForToken(tokenOption.address) }),
        'desc',
      )[0].address;
